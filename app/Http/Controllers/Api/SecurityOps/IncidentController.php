...
namespace App\Http\Controllers\Api\SecurityOps;
...
class IncidentController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        // Build filters array for export
        $filters = [];
        if ($request->filled('category_id')) {
            $filters['category_id'] = $request->string('category_id')->toString();
        }
        if ($request->filled('severity')) {
            $filters['severity'] = $request->string('severity')->toString();
        }
        if ($request->filled('status')) {
            $filters['status'] = $request->string('status')->toString();
        }
        if ($request->filled('priority')) {
            $filters['priority'] = $request->string('priority')->toString();
        }
        if ($request->filled('from')) {
            $filters['from'] = $request->date('from')->toDateString();
        }
        if ($request->filled('to')) {
            $filters['to'] = $request->date('to')->toDateString();
        }
        if ($q = $request->string('q')->toString()) {
            $filters['q'] = $q;
        }

        return Excel::download(new IncidentsExport($filters, $user, $employee), $filename);
    }
}
