...
export default function ClientProjectEdit({ project, masters }: Props) {
  const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Dashboard', href: route('dashboard') },
    { title: 'Projek Klien', href: route('crm.client-projects.index') },
    { title: `Edit ${project.name}`, href: route('crm.client-projects.edit', { clientProject: project.id }) },
  ];

  const { data, setData, put, processing, errors } = useForm({
    client_id: project.client_id?.toString() ?? '',
    outsourcing_field_id: project.outsourcing_field_id?.toString() ?? '',
    code: project.code ?? '',
    name: project.name ?? '',
    ...
    required_agents: project.required_agents ?? 1,
    status: project.status ?? 'tender',
    contract_start: project.contract_start?.substring(0, 10) ?? '',
    contract_end: project.contract_end?.substring(0, 10) ?? '',
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('crm.client-projects.update', { clientProject: project.id }));
  };

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={`Edit Projek: ${project.name}`} />
      ...
      <div className="mx-auto w-full max-w-[1080px] px-4 pt-4 pb-28">
        ...
        <Card className="border-border/60">
          <CardHeader>
            <CardTitle>Formulir Edit Projek</CardTitle>
            <CardDescription>Perbarui data projek di bawah ini lalu simpan perubahan.</CardDescription>
          </CardHeader>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    ...
                    <Select value={data.client_id} onValueChange={(v) => setData('client_id', v)} disabled>
                      <SelectTrigger>
